...
func (r *ChannelRelay) RelayComment(ctx context.Context, comment *models.TaskComment) error {
	...
	// Channel relay fires from the comment-bridging path triggered by an
	// agent turn — the originating run is whichever one is currently
	...
	r.svc.LogActivityWithRun(ctx, channel.WorkspaceID, "system", "", "channel.message_relayed",
		"channel", channel.ID,
