...
// Auditable reports whether this authorization decision created a durable
// audit event. Callers must finish only auditable decisions, so ordinary
// denials and direct-parent access never create synthetic audit updates.
func (d Decision) Auditable() bool {
	return d.AuditID != ""
}
...
func (a *Authority) Finish(ctx context.Context, decision Decision, operationErr error) error {
	if a == nil || a.store == nil || !decision.Auditable() {
		return nil
	}
	result := "ok"
	detail := ""
