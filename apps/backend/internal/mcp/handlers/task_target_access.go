...
func (h *Handlers) authorizeCoordinatorAction(ctx context.Context, caller, target *models.Task, action string, capability coordinator.Capability) (coordinator.Decision, error) {
	...
	return h.coordinatorAuthority.Authorize(ctx, coordinator.Request{ActorTask: caller, TargetTask: target, Action: action, Capability: capability})
}
