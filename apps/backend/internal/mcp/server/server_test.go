...
func TestServerModeTask_RegistersCorrectTools(t *testing.T) {
	...
	assert.Contains(t, tools, "recover_session_queue_kandev")
	assert.Contains(t, tools, "close_task_session_kandev")
...
func TestServerSurfaceAutomationHasFixedCoordinatorCatalog(t *testing.T) {
	...
	want := []string{
		...
		"recover_session_queue_kandev", "close_task_session_kandev",
...
drained:
	...
	require.Len(t, tools, 40, "final registry should contain the complete GitLab-only task tool set")
	assert.Contains(t, tools, "get_task_mr_automation_kandev")
	assert.NotContains(t, tools, "get_task_pr_automation_kandev")
}
...
func TestServerModeTask_ToolCount(t *testing.T) {
	...
	// + 2 task-dependency (add/remove) + 1 rich-output + 2 guarded session cleanup tools = 42.
