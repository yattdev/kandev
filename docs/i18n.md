...
European Portuguese (`pt-pt`) and Simplified Chinese (`zh-cn`) are the shipped
...
1. Add the locale to `SUPPORTED_LOCALES` and give it a fixed endonym in
   ...
   and backend lists synchronized.
2. Copy every `apps/web/src/locales/en/*.json` namespace into
   `apps/web/src/locales/<locale>/`. Translate values only: never rename keys,
   plural suffixes, interpolation placeholders, `<Trans>` tags, command tokens,
   shortcuts, filenames, URLs, or brand names.
...
Kandev's migration remains incremental. A complete locale catalog guarantees
