...
| Agent-spawned sessions and targeted task messages | Dependency-bound | `spawn_session_kandev` and `message_task_kandev` are task-MCP tools. Queue limits, session IDs, parent/child interrupt rules, and agent MCP capability apply. |
